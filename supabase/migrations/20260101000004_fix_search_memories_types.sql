-- Fix search_memories RETURN QUERY type mismatch
-- pgvector <=> returns double precision; function declares REAL columns
  ...
  SELECT
    ...
    (
      COALESCE(CASE WHEN p_query_embedding IS NOT NULL AND m.embedding IS NOT NULL
        THEN ((1 - (m.embedding <=> p_query_embedding)) * 0.6)::REAL
        ELSE 0::REAL END, 0) +
      COALESCE((ts_rank(to_tsvector('english', m.original_text), plainto_tsquery('english', p_query)) * 0.4)::REAL, 0)
